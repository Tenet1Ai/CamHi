...
- (void)requestWithCommand:(int)cmd date:(char *)data size:(int)size information:(id)info {
    
    [HXProgress showProgress];
    [self sendIOCtrl:cmd Data:data Size:size];
}
...
- (void)receiveIOCtrl:(HiCamera *)camera Type:(int)type Data:(char*)data Size:(int)size Status:(int)status {
    ...
    /********** 温湿度报警 **********/
    if (type == HI_P2P_TEMPERATURE_ALARM_GET) {
        self.gmTemperature = [[MTemperature alloc] initWithData:data size:size];
        [self returnGetBlockWithCommand:type object:self.gmTemperature information:nil];
    }
    
    if (type == HI_P2P_TEMPERATURE_ALARM_SET) {
        [self returnSetBlockWithCommand:type size:size object:nil information:nil];
    }

    if (type == HI_P2P_HUMIDITY_ALARM_GET) {
        self.gmHumidity = [[MHumidity alloc] initWithData:data size:size];
        [self returnGetBlockWithCommand:type object:self.gmHumidity information:nil];
    }
    
    if (type == HI_P2P_HUMIDITY_ALARM_SET) {
        [self returnSetBlockWithCommand:type size:size object:nil information:nil];
    }
...
#pragma mark -- new block
- (void)returnGetBlockWithCommand:(NSInteger)cmd object:(id)obj information:(id)info {
    
    dispatch_async(dispatch_get_main_queue(), ^{
       
        [HXProgress dismiss];
        
        if (_returnCommandBlock) {
            
            if (obj) {
                _returnCommandBlock(YES, cmd, obj, info);
            }
            if (!obj) {
                _returnCommandBlock(NO, cmd, nil, info);
            }
        }
...
- (void)returnSetBlockWithCommand:(NSInteger)cmd size:(NSInteger)size object:(id)obj information:(id)info {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_returnCommandBlock) {
            ...
            if (size < 0) {
                _returnCommandBlock(NO, cmd, obj, info);
            }
        }
...
- (void)requestTemperatureAndHumidity {
    [self requestWithCommand:HI_P2P_TEMPERATURE_ALARM_GET date:(char *)nil size:0 information:nil];
    [self requestWithCommand:HI_P2P_HUMIDITY_ALARM_GET date:(char *)nil size:0 information:nil];
}
// 开启温度报警
- (void)enableTemperatureAlarm:(BOOL)enable {
    
    if (!self.gmTemperature) {
        LOG(@"self_gmTemperature_is_nil.")
        return;
    }
    
    if (self.gmTemperature) {
        
        self.gmTemperature.u32Enable = enable ? 1 : 0;
        HI_P2P_TMP_ALARM *tmp_alarm = [self.gmTemperature model];
        [self requestWithCommand:HI_P2P_TEMPERATURE_ALARM_SET date:(char *)tmp_alarm size:sizeof(HI_P2P_TMP_ALARM) information:nil];
        free(tmp_alarm);
    }
}
// 开启湿度报警
- (void)enableHumidityAlarm:(BOOL)enable {
    ...
    if (!self.gmHumidity) {
        LOG(@"self_gmHumidity_is_nil.")
        return;
    }
    
    if (self.gmHumidity) {
        
        self.gmHumidity.u32Enable = enable ? 1 : 0;
        HI_P2P_HUM_ALARM *hum_alarm = [self.gmHumidity model];
        [self requestWithCommand:HI_P2P_HUMIDITY_ALARM_SET date:(char *)hum_alarm size:sizeof(HI_P2P_HUM_ALARM) information:nil];
        free(hum_alarm);
    }
}
...
//            temps = @[@"AAES", @"aaes"];
            ...
            temps = @[@"AAAA", @"BBBB", @"CCCC", @"DDDD", @"EEEE", @"FFFF"];
//            temps = @[@"AAAA", @"BBBB", @"CCCC", @"DDDD", @"EEEE"];
        }
