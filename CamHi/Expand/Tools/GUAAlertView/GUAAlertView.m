...
+ (instancetype)alertViewWithTitle:(NSString *)title
    ...
    return [[GUAAlertView alloc] initWithTitle:title view:view buttonTitle:buttonTitle buttonTouchedAction:buttonAction dismissAction:dismissAction];
}
//+ (instancetype)alertViewWithTitle:(NSString *)title
...
//                            @"content": _messageLabel,
    ...
    [_alertView addConstraints:[NSLayoutConstraint
                                ...
                                @"V:|-padding-[title]-padding-[button(==buttonHeight)]|"
                                options:0
                                metrics:metrics
                                views:views]];
...
- (void)setupContent {
    UILabel *label = [self labelWithText:@"msg"];
    label.font = [UIFont systemFontOfSize:14];
    
    [_alertView addSubview:label];
    _messageLabel = label;
    
//    UIProgressView *progressView_ = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleDefault];
//    _progressView.progress = 0.5;
    [_alertView addSubview:_progressView];
//    _progressView.frame =
    _progressView.frame = CGRectMake(25, 80, 225, 30);

//    _progressView = progressView_;
}
...
- (void)dismiss {
    ...
    if (_dismissBlock != NULL) {
        ...
        if(_clickeddelegate) {
            [_clickeddelegate clickedButtonAtIndex:0];
        }
